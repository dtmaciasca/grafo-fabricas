...
import com.uniandes.lps.grafo.dominio.Arco;
import com.uniandes.lps.grafo.dominio.Nodo;

public class GrafoConBFS implements Grafo {
	...
	private boolean buscarRutaBFS(List<Nodo> nodosRuta, Nodo nodoOrigen, Nodo nodoDestino) {
        ...
        while(!queue.isEmpty()){
            ...
            if(actual.equals(nodoDestino)) {
                ...
                if(actual.getNodosAdyacentesBFS().isEmpty())
                    return false;
                else {
                	for (Nodo nodo: actual.getNodosAdyacentesBFS()) {
